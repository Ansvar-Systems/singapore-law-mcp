# Singapore Law MCP Server — Developer Guide
...
Singapore Law MCP server providing English-language Singapore federal legislation search via Model Context Protocol. Strategy A deployment (Vercel, bundled SQLite DB).
...
- Singapore uses "Section" numbering (not "Article") — provision_ref prefix is "s" (e.g., "s2")
- Citation format: "Section N [Act Title Year]" or "s N" (common law style)
...
1. `scripts/ingest.ts` → fetches from SSO (sso.agc.gov.sg) → JSON seed files in `data/seed/`
...
- **Singapore Statutes Online (SSO)** (sso.agc.gov.sg) — Attorney-General's Chambers of Singapore
- **License:** Singapore Open Data Licence
- **Language:** English (sole legislative language)
- **Coverage:** All current Acts of Parliament, subsidiary legislation, Constitution of the Republic of Singapore

## Deployment

- Vercel Strategy A: DB bundled in `data/database.db`, included via `vercel.json` includeFiles
- npm package: `@ansvar/singapore-law-mcp` with bin entry for stdio
