...
export async function getSingaporeImplementations(
  db: InstanceType<typeof Database>,
  input: GetSingaporeImplementationsInput,
): Promise<ToolResponse<SingaporeImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as SingaporeImplementationResult[];
