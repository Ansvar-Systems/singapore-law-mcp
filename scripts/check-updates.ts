...
const PORTAL_URL = 'https://sso.agc.gov.sg';
const PORTAL_NAME = 'Singapore Statutes Online';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/singaporean-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Singaporean Law MCP — Data Freshness Check');
